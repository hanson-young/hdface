...
class hdface_detector(object):
    ...
    def __init__(self,
        ...
        self.lnet_detector = None

        pnet, rnet, onet, lnet = self.hdface_init()
        if self.stage == 1:
    ...
    def hdface_init(self):
        ...
        if p_model_path is not None:
            pnet = PNet(use_cuda=self.use_cuda)
            if (self.use_cuda):
        ...
        if r_model_path is not None:
            rnet = RNet(use_cuda=self.use_cuda)
            if (self.use_cuda):
        ...
        if o_model_path is not None:
            onet = ONet(use_cuda=self.use_cuda)
            if (self.use_cuda):
        ...
        if l_model_path is not None:
            lnet = ONet(use_cuda=self.use_cuda)
            if (self.use_cuda):
                lnet.load_state_dict(torch.load(l_model_path))
                lnet.cuda()
            else:
                lnet.load_state_dict(torch.load(l_model_path, map_location=lambda storage, loc: storage))
            lnet.eval()
    ...
    def detect_pnet(self, im):
        ...
        while min(current_height, current_width) > net_size:
            ...
            cls_map, reg = self.pnet_detector((feed_imgs - 127.5) / 128.)
            # cls_map, reg = self.pnet_detector(feed_imgs / 255.)
            ...
            # detla_w = boxes[:,2] - boxes[:,0]
            # detla_h = boxes[:,3] - boxes[:,1]
...
face candidates:%d, current batch_size:%d"%(num_boxes, batch_size)
        ...
        cls_map, reg = self.rnet_detector((feed_imgs - 127.5) / 128.)
    ...
    def detect_onet(self, im, dets):
        ...
        cls_map, reg, landmark = self.onet_detector((feed_imgs - 127.5) / 128.)
    ...
    def detect_lnet(self, img):
        cropped_ims_tensors = []
        re_img = cv2.resize(img, (48, 48))
        crop_im_tensor = image_tools.convert_image_to_tensor(re_img)
        # cropped_ims_tensors[i, :, :, :] = crop_im_tensor
        cropped_ims_tensors.append(crop_im_tensor)
        feed_imgs = Variable(torch.stack(cropped_ims_tensors))

        if self.lnet_detector.use_cuda:
            feed_imgs = feed_imgs.cuda()

        cls_map, reg, landmark = self.lnet_detector((feed_imgs - 127.5) / 128.)
